...
module Hand = {
  ...
  let make = props => {
    let angle = _ => 2.0 * Math.Constants.pi * (props.turns - 0.25)
    let x = _ => (props.size / 2.0 + props.length * angle()->Math.cos)->Float.toString
    let y = _ => (props.size / 2.0 + props.length * angle()->Math.sin)->Float.toString
...
let make = props => {
  let s = _ => props.size->Float.toString
  ...
  let hour = _ => time()->Date.getHours->Int.toFloat / 12.0
  let minute = _ => time()->Date.getMinutes->Int.toFloat / 60.0
  let seconds = _ => time()->Date.getSeconds->Int.toFloat / 60.0
  ...
  <svg width={s()} height={s()} viewBox={`0 0 ${s()} ${s()}`}>
    ...
    <Hand handColor={props.handColor} size={props.size} width=3 length=90.0 turns={seconds()} />
  </svg>
}
